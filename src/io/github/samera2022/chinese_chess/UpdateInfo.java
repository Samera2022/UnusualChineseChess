...
public enum UpdateInfo {
    ...
    VERSION_1_3_0("1.3.0","2026-01-12 ",
                    "## [Added]\n" +
                    ...
                    "## [Changed]\n" +
                    " - “强制走子”被限制在仅联机时可用。\n" +
                    " - “强制走子”的指示器可能会发生样式上的改变。\n\n" +
                    "## [Note]\n" +
                    " - 由于作者的疏忽，在2026-01-11的凌晨忘记将写好的功能提交到github的repo并直接开始重构逻辑，导致昨晚的进度在很大程度上丢失了。又因为release和提交时间不同步的缘故，" +
                    "导致部分1.2.0的功能原先实现已被丢失，且对反编译的尝试被证实是艰难的。（大量的引用被改为常量，且反编译得到的结果不利于开发和维护）因此，部分1.2.0的功能实为01-11重新实现的，" +
                    "进而会与1.2.0的功能在细节上有所出入。");
