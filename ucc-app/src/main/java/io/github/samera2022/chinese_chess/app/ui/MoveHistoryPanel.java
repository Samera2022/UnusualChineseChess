 ...
 * 着法记录面板 - 显示游戏的着法历史
 */
public class MoveHistoryPanel extends JPanel implements SessionListener {
    ...
    public MoveHistoryPanel(GameSession session) {
    ...
    public void showNavigation() {
        List<Move> moves = session.getMoveHistory();
        if (!moves.isEmpty()) {
            isInReplayMode = true;
            currentStep = moves.size(); // 初始显示最后一步
            session.setReplayMode(true, currentStep);
    ...
    private void updateMoveHistory() {
        ...
        List<HistoryItem> history = session.getCombinedHistory();
